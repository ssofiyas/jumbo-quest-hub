...
import { Settings, Phone, Mail, MapPin, Globe, Bell, Moon, ChevronRight, ArrowUp, ArrowDown, LogOut, Award } from "lucide-react";
...
export const ProfileScreen = ({ name }: { name: string }) => {
  ...
  return (
    <div className="h-full overflow-y-auto pb-24 scrollbar-hide animate-slide-up">
      ...
      <div className="relative gradient-hero pt-2 pb-20 px-5">
        ...
        <div className="flex items-center gap-4 mt-6">
          ...
          <div className="text-primary-foreground">
            ...
            <p className="text-xs opacity-90">ID · JMB-8421-ANNA</p>
      ...
      {/* Personal info */}
      <section className="px-5 mb-6">
        <h3 className="text-base font-bold mb-3">Personal details</h3>
        <div className="bg-card rounded-2xl shadow-soft divide-y divide-border">
          {[
            { icon: Phone, label: "Phone", value: "+358 40 123 4567" },
            { icon: Mail, label: "Email", value: "anna.k@example.fi" },
            { icon: MapPin, label: "Address", value: "Vantaa, Finland" },
          ].map((r, i) => {
            const Icon = r.icon;
            return (
              <div key={i} className="flex items-center gap-3 p-3.5">
                <span className="h-9 w-9 rounded-full bg-secondary flex items-center justify-center text-primary shrink-0">
      ...
      {/* App settings */}
      <section className="px-5 mb-6">
        <h3 className="text-base font-bold mb-3">App settings</h3>
        <div className="bg-card rounded-2xl shadow-soft divide-y divide-border">
          {/* Language */}
          <div className="flex items-center gap-3 p-3.5">
            <span className="h-9 w-9 rounded-full bg-secondary flex items-center justify-center text-primary"><Globe className="h-4 w-4" /></span>
            <p className="flex-1 text-sm font-semibold">Language</p>
            <div className="flex bg-secondary rounded-full p-0.5">
              {(["EN", "FI"] as const).map((l) => (
                <button
                  ...
                  onClick={() => setLang(l)}
                  className={cn(
                    "text-[11px] font-bold px-3 py-1 rounded-full transition-smooth",
                    lang === l ? "bg-primary text-primary-foreground shadow-soft" : "text-muted-foreground",
                  )}
                >
                  {l}
                </button>
...
const Toggle = ({
  ...
  <div className="flex items-center gap-3 p-3.5">
    <span className="h-9 w-9 rounded-full bg-secondary flex items-center justify-center text-primary">
      <Icon className="h-4 w-4" />
    </span>
